...
TypeMarker = Union[str, Tuple[str, dict]]  # e.g., "int" or ("object", {...}) or ("array<object>", {...})

def _union_str(types: Iterable[str]) -> str:
    ts = sorted(set(types))
    return " | ".join(ts) if len(ts) > 1 else (ts[0] if ts else "unknown")

def guess_type(value: Any, max_array: int = 200) -> set[TypeMarker]:
    if value is None:
        return {"null"}
    ...
    if isinstance(value, list):
        ...
        if obj_items:
            ...
            leaf_types = [t for t in tset if isinstance(t, str)]
            return {f"array<{_union_str(leaf_types)}>"}
    if isinstance(value, dict):
        return {("object", value)}
    return {"unknown"}

def merge_objects(objs: List[dict], sample_n: int = 200, max_array: int = 200) -> Dict[str, Any]:
    key_counts: Counter[str] = Counter()
    field_vals: defaultdict[str, list] = defaultdict(list)
